<?php
// รับค่าพิกัดจาก URL หรือใช้ค่าพื้นฐานหากไม่มีการส่งค่ามา
$lat_cam = isset($_GET['lat_cam']) ? floatval($_GET['lat_cam']) : 14.439606;
$long_cam = isset($_GET['long_cam']) ? floatval($_GET['long_cam']) : 101.372359;
$lat_ele = isset($_GET['lat_ele']) ? floatval($_GET['lat_ele']) : 14.439606;
$long_ele = isset($_GET['long_ele']) ? floatval($_GET['long_ele']) : 101.372359;
?>
<!DOCTYPE html>
<html lang="th">
...
<body>
    ...
    <script>
        ...
        const cameraIcon = L.icon({
            iconUrl: "https://cdn-icons-png.flaticon.com/128/45/45010.png", // ไอคอนกล้อง
            iconSize: [30, 30],  // ขนาดของไอคอน
            iconAnchor: [15, 30], // จุดที่เชื่อมต่อกับตำแหน่งของมาร์กเกอร์
            popupAnchor: [0, -30] // จุดที่ popup จะปรากฏ
        });

        // ไอคอนสำหรับช้าง
        const elephantIcon = L.icon({
            iconUrl: "https://cdn-icons-png.flaticon.com/128/1864/1864469.png", // ไอคอนช้าง
            iconSize: [32, 32], // ขนาดของไอคอน
            iconAnchor: [16, 32], // จุดที่เชื่อมต่อกับตำแหน่งของมาร์กเกอร์
            popupAnchor: [0, -32] // จุดที่ popup จะปรากฏ
        });

        // ฟังก์ชันสำหรับเริ่มต้นแผนที่
        function initializeMap() {
            const initialLat = <?= $lat_cam ?>;
            const initialLng = <?= $long_cam ?>;
            mymap = L.map("mapid").setView([initialLat, initialLng], 13);
            ...
            // มาร์กเกอร์สำหรับกล้อง
            L.marker([<?= $lat_cam ?>, <?= $long_cam ?>], { icon: cameraIcon }).addTo(mymap)
                .bindPopup(`กล้อง CCTV<br>ละติจูด: <?= $lat_cam ?>, ลองจิจูด: <?= $long_cam ?>`)
                .openPopup();

            // มาร์กเกอร์สำหรับช้าง (ถ้ามี)
            if (<?= $lat_ele ?> && <?= $long_ele ?>) {
                L.marker([<?= $lat_ele ?>, <?= $long_ele ?>], { icon: elephantIcon }).addTo(mymap)
                    .bindPopup(`ช้าง<br>ละติจูด: <?= $lat_ele ?>, ลองจิจูด: <?= $long_ele ?>`)
                    .openPopup();
            }

            // เพิ่มการค้นหาตำแหน่ง (geocoder)
        ...
        async function fetchElephantData() {
            try {
                ...
                const validMarkers = data.data.filter(elephant => elephant.elephant === true).length;
                document.getElementById("nearbyCount").textContent = `${validMarkers} จุด`;

                // อัปเดตสถานะการตรวจจับช้าง
                const elephantStatus = latestDetection.elephant ? "เจอช้าง" : "ไม่เจอช้าง";
                const elephantStatusValue = latestDetection.elephant ? 1 : 0;
                document.getElementById("elephantStatus").textContent = elephantStatus;
                document.getElementById("elephantStatus").setAttribute("data-value", elephantStatusValue);
            } catch (error) {
                console.error("เกิดข้อผิดพลาดในการดึงข้อมูล:", error);
                showNotification("ไม่สามารถดึงข้อมูลช้างป่าได้ กรุณาลองใหม่อีกครั้ง", "error");
            } finally {
                hideLoading();
            }
        }
        ...
        function updateMap(detection) {
            ...
            if (!isNaN(lat) && !isNaN(lng)) {
                ...
                const timestamp = new Date(detection.timestamp).toLocaleString("th-TH", {
                    year: "numeric",
                    month: "long",
                    day: "numeric",
                    hour: "2-digit",
        ...
        async function checkNewData() {
            try {
                ...
                const latestDetection = data.data[0];
                if (latestDetection.timestamp && new Date(latestDetection.timestamp).getTime() > lastDetectionTime) {
        ...
        function toggleTheme() {
            ...
            // เปลี่ยนสีของ Route ตามโหมดธีม
            if (currentRoute) {
                mymap.removeControl(currentRoute);
                currentRoute = L.Routing.control({
                    waypoints: currentRoute.getWaypoints().map(wp => wp.latLng),
                    router: L.Routing.osrmv1({
                        serviceUrl: "https://router.project-osrm.org/route/v1",
                        profile: "driving",
                    }),
